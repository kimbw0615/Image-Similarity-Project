...
학습된 `SegFormer` 모델을 불러온 후 해당 이미지를 적용한 결과입니다.
   ...
   이미지으로부터 Feature를 추출하기 위해 `ResNet` 모델 사용
   ...
   모델의 이미지과 수집한 이미지들의 유사성을 확인하기 위해 코사인 유사도 계산
...
- 회고
	...
	- 김백운: 프로젝트를 통해 많은 것을 배우고 성장했습니다. 초기 목표를 90% 이상 달성하여 만족스럽고, 특히 어려움을 해결하는 과정에서 팀원과 협력 능력을 향상시켰습니다. 이전보다 적극적으로 참여하고 새로운 기술을 경험했지만, 딥러닝 모델에 대한 이해가 부족하다는 점을 느꼈습니다.
	- 김재원:  
